(() => {
  ...
  function describe(kind, data, error = null, now = Date.now()) {
    const rawDate = kind === "daily" ? data?.checked_at : kind === "evos" ? data?.verified_at : data?.generated_at || data?.updated_at;
    ...
    const evoReviewExpired = kind === "evos" && (!Number.isFinite(Date.parse(data?.review_after)) || now >= Date.parse(data.review_after));
    const stale = Boolean(error) || dailyExpired || evoReviewExpired || ageMs === null || ageMs > limit;
