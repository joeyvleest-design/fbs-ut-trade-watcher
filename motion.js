(() => {
  ...
  const labels = { daily: "Trade van de dag", market: "Marktprijzen", marketwatch: "Marketwatch", meta: "Meta-vrouwen", releases: "Releases & Gold / Promo", evos: "Nederlandse EVO-keuzes", legends: "Icons, Heroes & Hall of FUT", radar: "Geruchten & Bronradar" };
  ...
  function renderUpdates() {
    ...
    const items = Object.entries(labels).map(([kind, label]) => {
      ...
      if (kind === "evos" && state?.data && !state.error) description = state.stale || Date.now() >= Date.parse(state.data.review_after) ? "EVO-shortlist · hercontrole nodig" : "Databasecheck · niet in-game getest";
